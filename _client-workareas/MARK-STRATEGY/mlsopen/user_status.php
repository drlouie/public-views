...
if (($mls_uname == $uname) and ($mls_upass == $upass) and ($found == 1)) {
	...
	print "<br><center><h2>Update Listing Status</h2></center>\n";
	print "<div align=center><table border=1  bgcolor=\"$table_bg_color\">
	<tr><td><center><b><font color=\"$table_text_color\">MLS Number</font></b></center></td><td><center><b><font color=\"$table_text_color\">Title</font></b></center></td><td><center><b><font color=\"$table_text_color\">Listing Price</font></b></center></td><td><center><b><font color=\"$table_text_color\">City</font></b></center></td><td><center><b><font color=\"$table_text_color\">Status</font></b></center></td><td></td></tr>\n";

	$linkID = @mysql_connect("$server", "$username", "$password");
	mysql_select_db("$database", $linkID);

	$resultID = mysql_query("SELECT mls, realtor_user_name, title, price, property_city, status FROM user_mls", $linkID);
	...
	for ($x= 0; $x < mysql_num_rows($resultID); $x++) {
		...
		if ($row[realtor_user_name] == $uname) {
			...
			print "<tr><td><font color=\"$table_text_color\">$row[mls]</font></td><td><font color=\"$table_text_color\">$row[title]</font></td><td><font color=\"$table_text_color\">\$$row[price]</font></td><td><font color=\"$table_text_color\">$row[property_city]</font></td><td><font color=\"$table_text_color\">$status</font></td><td><form action=user_status2.php method=post><input type=hidden name=uname value=\"$uname\"><input type=hidden name=upass value=\"$upass\"><input type=hidden name=mls value=\"$row[mls]\"><input type=submit value=\"Update Status\"></form></td></tr>\n";
			$found = 1;
		}
	}
