...
feature35,feature36,feature37,feature38,feature39,feature40 FROM user_mls", $linkID);
	...
	for ($x= 0; $x < mysql_num_rows($resultID); $x++) {
		...
		if (($option_type == "Bedrooms") and ($option == $row[bedrooms])) {
		...
		if (($option_type == "Baths") and ($option == $row[baths])) {
		...
		if (($option_type == "Half-Baths") and ($option == $row[half_baths])) {
		...
		if (($option_type == "Square Feet") and ($option >= $row[sqf])) {
		...
		if (($option_type == "Acres") and ($option >= $row[arces])) {
