<form action="?" method="post" id="toolbar_form">
<input type="hidden" name="act" value="toolbar"  />
...
<?
	...
	if ($done)
	{
		...
		if (!$is_err)
		{
			foreach($caption as $tId => $c)
			{
				...
				$SQL->Query("UPDATE toolbar SET caption='$c', url='$u' WHERE id = $tId");
			}
		}
	}
	...
	if (!$done || $is_err)
	{
		...
		$db = $SQL->Query("SELECT id,caption,url FROM toolbar");
...
END;
		...
		$window['caption'] = 'Taskbar settings';
		$window['inner'  ] = $ret;
