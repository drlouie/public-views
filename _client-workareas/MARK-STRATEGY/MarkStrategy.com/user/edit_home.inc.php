...
<?
	...
	if ($done)
	{
		...
		if (!$is_err)
		{
			...
			KRF_StoreToolbarSettings($id_home, $url, $SQL);
	...
	{*/
		...
		$window['caption'] = 'Featured listing';
		...
		$window['inner'  ] .= '<br /><b>Taskbar settings</b>&nbsp;<span style="color:red">(leave blank if you want to use default url value else input url value with "http://" prefix)</span><br />'.
							KRF_ShowToolbarForm($SQL, $url = (is_array($url)) ? $url : NULL, $id_home);
...
BUTTONS;
	...
	{
		echo '<script>document.location.href="'.$main['root.com'].'user/?act=supply_user";</script>';
	}*/
?>