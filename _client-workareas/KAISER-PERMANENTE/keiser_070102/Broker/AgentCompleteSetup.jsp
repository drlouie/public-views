...
<jsp:useBean id="agentVO" class="org.kp.broker.vo.AgentVO" />
...
<%
	...
	agentVO = (org.kp.broker.vo.AgentVO)brokerVO.getAgent();
...
function init()
	{	
		document.AgentCompleteSetup.PhoneList.style.visibility = "hidden";	
	...
	<%}%>	
			...
			document.AgentCompleteSetup.BusinessType.value = '';
			document.AgentCompleteSetup.BroadcastMedium.value = '';
			document.AgentCompleteSetup.Country.value = '<%=addressVO.getCountry() %>';	
			
			state = '<%=addressVO.getState() %>';
			setState( document.AgentCompleteSetup );
		...
		document.AgentCompleteSetup.changeType[0].checked = true;		
	}
...
function resetForm()
{
			document.AgentCompleteSetup.reset();
			document.AgentCompleteSetup.BusinessType.value = '';
			document.AgentCompleteSetup.BroadcastMedium.value = '';
			document.AgentCompleteSetup.Country.value = '<%=addressVO.getCountry() %>';		
			document.AgentCompleteSetup.State.value = '<%=addressVO.getState()%>';			
}
</SCRIPT>
...
function submitDestinationForm() {
	...
	if ( parent.destination != undefined ) {
		if ( parent.destination.document.forms[0] != undefined ) {
			...
			parent.destination.document.forms[0].SaveExitPage.value = document.AgentCompleteSetup.SaveExitPage.value;
			...
			if ( parent.destination.document.forms[0].name == "AddTypeForm" ) {
				if ( validate(parent.destination.document.forms[0]) ) {
					...
					document.AgentCompleteSetup.WhereToGo.value="AdditionalAddress.jsp";					
					return true;
				}
				else
					return false;
				...
				if ( validateContactForm(parent.destination.document.forms[0]) ) {
					parent.destination.document.forms[0].TypeChangeKey.value = "Submit";
					//parent.destination.document.forms[0].submit();
					document.AgentCompleteSetup.WhereToGo.value="ContactPersonInfo.jsp";										
					return true;
				}
				else
					return false;
				...
				//parent.destination.document.forms[0].submit();
				document.AgentCompleteSetup.WhereToGo.value="Relationship.jsp";				
				return true;
			}
				...
				if ( validateDOIValues(parent.destination.document.forms[0]) ) {
					...
					parent.destination.document.forms[0].TypeChangeKey.value = "Submit";
					//parent.destination.document.forms[0].submit();
					document.AgentCompleteSetup.WhereToGo.value="License_DOI.jsp";
					return true;
				}
				else
					return false;
				...
				if ( parent.destination.document.forms[0].name == "AgreementsForm" )
					document.AgentCompleteSetup.WhereToGo.value="Agreements.jsp";				
				else if ( parent.destination.document.forms[0].name == "PaymentControlsForm" )
					document.AgentCompleteSetup.WhereToGo.value="PaymentControls.jsp";				
...
function enableAll(form)	//Enabling all the Fields when Edit clicked
 {
   var totalElements = document.AgentCompleteSetup.elements.length;
   for ( var i = 0 ; i <totalElements ; i++)
    {
	if(document.AgentCompleteSetup.elements[i].name != "bid" && document.AgentCompleteSetup.elements[i].name != "StatusDate" && document.AgentCompleteSetup.elements[i].name != "changeType"  && document.AgentCompleteSetup.elements[i].type != "button" && document.AgentCompleteSetup.elements[i].type != "submit" )
		{
			document.AgentCompleteSetup.elements[i].style.background = "white";
			document.AgentCompleteSetup.elements[i].onfocus = null;
		}
...
{	
	...
	for(var i=0 ; i < document.AgentCompleteSetup.PhoneList.options.length; i++) 
		{	
			var listValue = document.AgentCompleteSetup.PhoneList.options[i].value;
	...
	if(faxFound !=true && document.AgentCompleteSetup.EMail.value != "")
		{
			enableEMailInBroadcastMedium();
			faxFound = false;
		}
	else if(faxFound !=true && document.AgentCompleteSetup.EMail.value == "")
		{
			document.AgentCompleteSetup.BroadcastMedium.value = "";
			faxFound = false;
		}
...
{	
	document.AgentCompleteSetup.BroadcastMedium.value = "F";
}
function enableEMailInBroadcastMedium()
{	
	document.AgentCompleteSetup.BroadcastMedium.value = "E";
...
function checkForEMailAndFax(sel)
{
	...
	{
		if(document.AgentCompleteSetup.EMail.value != "")
		canSelect = true;
	}
  if(canSelect != true)
	{
		alert('<%=org.kp.base.web.util.MessageUtil.getInstance().getText("broker.BroadCaseMediumInvalid")%>');
		document.AgentCompleteSetup.BroadcastMedium.value = document.AgentCompleteSetup.PreviousBrdcst.value;
	}	
}

function disableDropDowns()
{
	document.AgentCompleteSetup.Status.disabled = true;			
	document.AgentCompleteSetup.StatusReason.disabled = true;
	document.AgentCompleteSetup.BrokerCategory.disabled = true;
	document.AgentCompleteSetup.BrokerCategoryYear.disabled = true;
	document.AgentCompleteSetup.Country.disabled = true;
	document.AgentCompleteSetup.changeType[0].disabled = true;
	document.AgentCompleteSetup.changeType[1].disabled = true;	
	
}
function enableDropDowns()
{
	document.AgentCompleteSetup.Status.disabled = false;			
	document.AgentCompleteSetup.StatusReason.disabled = false;
	document.AgentCompleteSetup.BrokerCategory.disabled = false;
	document.AgentCompleteSetup.BrokerCategoryYear.disabled = false;
	document.AgentCompleteSetup.Country.disabled = false;
	document.AgentCompleteSetup.State.disabled = false;
	document.AgentCompleteSetup.changeType[0].disabled = false;
	document.AgentCompleteSetup.changeType[1].disabled = false;	
...
{	
	document.AgentCompleteSetup.EffDateChanged = "changed";	
}
</SCRIPT>
...
<Script language="JavaScript">
   function callBrokerTypeChange() {
		...
		document.AgentCompleteSetup.action="ChangeBrokerTypeServlet";
		document.AgentCompleteSetup.target="_parent";
		document.AgentCompleteSetup.submit();
   ...
   function callComments() {
   		...
   		document.AgentCompleteSetup.sourceId1.value = document.AgentCompleteSetup.bid.value;

   		if ( funType == "" )
   			funType = "DMGR";
		
		var url = 'Comments/CommentServlet?sourceId1='+document.AgentCompleteSetup.sourceId1.value+'&sourceId2=0&sourceId3=0&sourceInd=BKR&funType='+funType;
...
function printFunction() {
   ...
   function callSearch() {
		...
		document.AgentCompleteSetup.action="SearchServlet";
		document.AgentCompleteSetup.submit();
   ...
   function callExit() {
	...
	if ( retValue == "Yes" ) {
		...
		document.AgentCompleteSetup.SaveExitPage.value = "Yes";
		if ( submitForm( document.AgentCompleteSetup) ) {
			document.AgentCompleteSetup.target="_parent";
			document.AgentCompleteSetup.submit();
		}

		/*
		document.AgentCompleteSetup.action="BaseMainMenu.htm";
		document.AgentCompleteSetup.target="_parent";
		document.AgentCompleteSetup.submit();
		*/
	}
		...
		document.AgentCompleteSetup.action="ExitServlet";
		document.AgentCompleteSetup.target="_parent";
		document.AgentCompleteSetup.submit();
	}
...
<BODY bgcolor="#0B5F77" onload="init();disableDropDowns()">
<FORM name="AgentCompleteSetup" method="post" action="AgentCompleteSetup" target="_parent" onsubmit="return submitForm(AgentCompleteSetup);">
        ...
        <table width="380" border="0" cellspacing="0" cellpadding="0">
          <tr> 
            ...
            <td bgcolor="#E6E9F0" width="374"><font class="maintitle" id="theTitle"> 
              <nobr>BKR101-Administer Broker Agent (Complete Setup)</nobr></font></td>
    ...
    <tr> 
      ...
      <td colspan="3" valign="top" height="35"> 
        <table width="125" border="0" cellspacing="0" cellpadding="0">
          ...
          <tr> 
            <td width="1" bgcolor="#E6E9F0"><img src="limages/spacer.gif" border="0" width="1" height="1"></td>
            <td height="20" width="198" align="center"><INPUT type="radio" checked name="changeType" value="agent"><FONT class="ComTitle">Agent</FONT><INPUT type="radio" name="changeType" value="firm" onclick="callBrokerTypeChange()"><FONT class="ComTitle">Firm</FONT></td>
            <td width="1" bgcolor="#E6E9F0"><img src="limages/spacer.gif" border="0" width="1" height="1"></td>
          </tr>
          <tr> 
            <td height="1" bgcolor="#E6E9F0"><img src="limages/spacer.gif" border="0" width="1" height="1"></td>
        ...
        <table width="840" border="0" cellspacing="0" cellpadding="1">
          ...
          <tr align="left" valign="middle" bgcolor="#E6E9F0"> 
            <td width="48" align="center"> 
              <input size="6" type="text" maxlength="12" name="bid" class="input1" onFocus="this.blur()" value="<%=brokerVO.getBID()%>">
            </td>
            <td width="75" align="center"> 
              <input size="11" type="text" maxlength="9" name="TaxID" onFocus="this.blur()" class="input1" value="<%=brokerVO.getTaxID()%>">
            </td>
...
function populateReason(selected)
	{	
		var listlength = 0 ;
		document.AgentCompleteSetup.StatusReason.options.length = 0;
			...
			{				
				if(statusReasonValue[count] == selected)	
					{	
						...
						document.AgentCompleteSetup.StatusReason.options[listlength] = reason;
						listlength+=1;
					}
			}
              ...
              <select name="Status" style="background-color : silver;" onChange="populateReason(this.value)">
                <%
		...
		%>
            ...
            <td width="125" align="center"> 
              <select name="StatusReason" style="background-color : silver;">
                <%
		...
		%>
            ...
            <td width="71" align="center"> 
              <input size="11" type="text" maxlength="10" name="StatusDate" value = '<%=statusVO.getStatusDate()%>' class="input1" onFocus="this.blur()">
            </td>
            <td width="154">&nbsp;</td>
...
function changeCategory(newValue)
	{		
		category[document.AgentCompleteSetup.BrokerCategoryYear.value]  = newValue;
		
	}
function changeCategoryYear(id)
	{	
		document.AgentCompleteSetup.BrokerCategory.value = category[id];
	}
</script>
      ...
      <td colspan="3" valign="top"> 
        <table width="790" border="0" cellspacing="0" cellpadding="1">
          ...
          <tr align="center" bgcolor="#E6E9F0"> 
            <td> 
              <input size="5" type="text" maxlength="10" name="Honor" class="input1" onFocus="this.blur()" value="<%=agentVO.getHonor()%>">
            </td>
            <td> 
              <input size="20" type="text" maxlength="25" name="FName" class="input1" onFocus="this.blur()" value="<%=agentVO.getFirstName()%>">
            </td>
            <td> 
              <input size="15" type="text" maxlength="25" name="MName" class="input1" onFocus="this.blur()" value="<%=agentVO.getMiddleName()%>">
            </td>
            <td> 
              <input size="20" type="text" maxlength="25" name="LName" class="input1" onFocus="this.blur()" value="<%=agentVO.getLastName()%>">
            </td>
            <td> 
              <input size="10" type="text" maxlength="20" name="Suffix" class="input1" onFocus="this.blur()" value="<%=agentVO.getSuffix()%>">
            </td>
            <td> 
              <input size="15" type="text" maxlength="20" name="Title" class="input1" onFocus="this.blur()" value="<%=agentVO.getTitle()%>">
            </td>
            <td> 
              <input size="15" type="text" maxlength="20" name="FamiliarName" class="input1" onFocus="this.blur()" value="<%=agentVO.getFamiliarName()%>">
            </td>
            <td> 
                    ...
                    <table width="353" border="0" cellspacing="0" cellpadding="2">
                      <tr valign="bottom"> 
                        <td align="center" width="176" valign="middle" height="27"><font class="ComTitle">Eff 
                          Date</font> 
                          <input size="11" type="text" maxlength="10" name="EffDate" class="input1" onFocus="this.blur()" value="<%=addressVO.getEffectiveDate()%>">
                        ...
                        <td align="left" colspan="3" valign="middle"> 
                          <input size="11" type="text" maxlength="20" name="MailStop" class="input1" onFocus="this.blur()" value="<%=addressVO.getMailStop()%>">
                        </td>
                      </tr>
                      <tr valign="bottom"> 
                        ...
                        <td width="37" height="27"> 
                          <!--<INPUT size="2" type="text" maxlength="2" name="State" style="background-color : silver;" onfocus="this.blur()" value="<%=addressVO.getState()%>"> &nbsp;-->
                      ...
                      <tr valign="bottom"> 
                        ...
                        <td width="187" colspan="3"> 
                          <select name="Country" onChange="setState(document.AgentCompleteSetup)" class="select1">
                            <% 
		...
		%>
                      ...
                      <tr valign="bottom"> 
                        <td align="right" width="176"><nobr><font class="ComTitle"><font style="color:#eb0000;font-size:11px">*</font>City</font>&nbsp; 
                          <input size="20" type="text" maxlength="25" name="City" class="input1" onFocus="this.blur()" value="<%=addressVO.getCity()%>">
                          </nobr> </td>
                  ...
                  <td width="373" align="center"> 
                    <table width="373" border="0" cellspacing="0" cellpadding="2">
                      ...
                      <tr align="center" valign="middle" bgcolor="#E6E9F0"> 
                        <td width="120"> 
                          <input size="20" type="text" class="input2" maxlength="40" name="EMail" tabindex="2"  onChange = "checkForFaxInPhoneList()">
                        </td>
                        <td width="120"> 
                          <input size="20" type="text" class="input2" maxlength="40" name="WebURL" tabindex="3">
                        </td>
                        <td width="133"> 
                          <select name="BroadcastMedium" class="select2" tabindex="11" onChange = "checkForEMailAndFax(this.value)" 
onClick = "document.AgentCompleteSetup.PreviousBrdcst.value = document.AgentCompleteSetup.BroadcastMedium.value;">
                            <% 
                    ...
                    <table width="373" border="0" cellspacing="0" cellpadding="2">
                      ...
                      <tr align="center" valign="bottom"> 
                        ...
                        <td width="25" bgcolor="#E6E9F0"> 
                          <input size="3" class="input2" type="text" maxlength="3" name="CountryCode" tabindex="5">
                        </td>
                        <td width="25" bgcolor="#E6E9F0"> 
                          <input size="3" class="input2" type="text" maxlength="3" name="AreaCode" tabindex="6">
                        </td>
                        <td width="30" bgcolor="#E6E9F0"> 
                          <input size="7" class="input2" type="text" maxlength="7" name="Number" tabindex="7">
                        </td>
                        <td width="25" bgcolor="#E6E9F0"> 
                          <input size="5" class="input2" type="text" maxlength="5" name="Ext" tabindex="8">
                        </td>
                        <td width="20" bgcolor="#E6E9F0"> 
                          <input type="checkbox" name="Preference" tabindex="9">
                        </td>
                        <td width="178" align="center" bgcolor="#E6E9F0"><a href="#" tabindex="29" OnClick="addToList(document.AgentCompleteSetup);" onMouseOver="javascript:add.src = image15.src;" onMouseOut="javascript:add.src = image16.src;"><img src="limages/add_off.gif" width="50" height="19" border="0" alt="Add" name="add"></a> 
                        </td>
                      </tr>
                      <tr align="center" valign="bottom"> 
                        <td colspan="7" height="50"> 
                          <select size="3" name="PhoneList" style="background-color:#ffffff" onClick="popAgain(PhoneList,this.form.PhoneList.value)" class="multiselect">
                          </select>
                        </td>
                      </tr>
                      <tr align="center" valign="bottom"> 
                        <td colspan="7" height="30"><a href="#" OnClick="deleteFromList(document.AgentCompleteSetup);" onMouseOver="javascript:delsel.src = image17.src;" onMouseOut="javascript:delsel.src = image18.src;"><img src="limages/delsel_off.gif" width="138" height="19" border="0" alt="Delete Selected" name="delsel" vspace="0"></a> 
                        </td>
                      </tr>
                    </table>
                  </td>
              ...
              <input type="hidden" name="GoTo" value="BrokerAgentAdminSys.jsp">
              ...
              <!--<INPUT size="12" type="text" maxlength="12" name="bid" style="background-color : silver;" onfocus="this.blur()" value="<%=brokerVO.getBID()%>"></TD>-->
              <!-- INCLUDED BY SATHISH -->
