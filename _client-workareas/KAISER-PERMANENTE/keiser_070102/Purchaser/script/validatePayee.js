...
function validate( theForm ) {
	...
	if ( theForm.payeeBrokerID.value == "" ) {
		errorFields += "\n" + "<%=org.kp.base.web.util.MessageUtil.getInstance().getText("purchaser.payee.PayeeIdRequired")%>";
		errorStatus = true;
	}
	  else{
          checkPayee(theForm.payeeBrokerID.name,theForm.payeeBrokerID.value);
            }  
	...
	if ( theForm.payeeShare.value == "" ) {
		errorFields += "\n" + "<%=org.kp.base.web.util.MessageUtil.getInstance().getText("purchaser.payee.PayeeShareRequired")%>";
		errorStatus = true;
	}
	 // else{
         // checkPayee(theForm.payeeShare.name,theForm.payeeShare.value);
          //  }
...
function checkDateValidation(datename, datevalue){
	...
	function checkPayee(name,value) {


		var Name = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ ";	
		var payeeID = "1234567890";
		var valid = "";
 
  		if( name == "payeeBrokerID" || name =="payeeShare" ) {
			valid = payeeID ;
		}
...
function validateBID(theForm)
{
	...
	if ( theForm.payeeBrokerID.value == "" ) {
		...
		checkBrokerID(theForm.payeeBrokerID.value);
	   }
...
function checkBrokerID(BrokerID)
 {
   ...
   for (i = 0;  i < checkStr.length;  i++)
    ...
    if (!allValid)
     {
        errorFields+= "<%=org.kp.base.web.util.MessageUtil.getInstance().getText("purchaser.payee.PayeeIdDigits")%>"+"\n";
    	 errorStatus =true;
     }
