...
// For use ONLY with AmericanMicroCorp.com                       //
...
function cm_makeMenu(name,parent,text,link,target,width,height,img1,img2,bgcoloroff,bgcoloron,textcolor,hovercolor,onclick,onmouseover,onmouseout){
	...
	else if(bw.ns4){text='<font face="'+(prop1.NS4font||prop2.NS4font)+'" color="'+(textcolor||this.l[l].tc)+'">'+text+'</font>'}
		...
		for(j=0;j<this.l[i].maxnum;j++){
			...
			if(i==0){
				...
				if(!bw.ns6){
					...
					str+=tempstr+' class="cl'+this.name+i+'"><center>'+txt+'</center></div>'; str+='</div>\n'
				}else this.oNS[j]=cm_NS6_createElement(this.ns6styleb[i],tempstr+' class="cl'+this.name+i+'"><center>'+txt+'</center></div>')
