...
else { $light1 = "CAD8E8"; $dark1 = "01458E"; $dark2 = "01458E"; }
...
##	}
	...
	###CUAL MAIN HEADER? QUEST >>> SEAGATE >>> ALL OTHERS
	if ($title eq "seagate" || $title eq "quest") {
		if ($title eq "seagate") { 
			if ($FORM{'loadthis'} eq "seagate") { $cualMainHeader = "<a href=\"sharktank.cgi?title=seagate&loadthis=seagate&SEAGATE=1\"><img src=\"images/seagate_headerL.jpg\" width=\"343\" height=\"107\" border=\"0\">"; }
			else { $cualMainHeader = "<a href=\"sharktank.cgi?title=seagate&loadthis=seagate&SEAGATE=1\"><img src=\"images/seagate_header.jpg\" width=\"240\" height=\"107\" border=\"0\"></a>"; }
			$cualCSS = "<LINK REL=\"STYLESHEET\" HREF=\"common_css.cgi?SEAGATE=1\" Type=\"text/css\">";
			...
			if ($FORM{'loadthis'} eq "quest") { $cualMainHeader = "<br><a href=\"sharktank.cgi?title=quest&loadthis=quest&QUEST=1\"><img src=\"images/quest_headerL.jpg\" width=\"255\" height=\"49\" border=\"0\">"; }
			else { $cualMainHeader = "<a href=\"sharktank.cgi?title=quest&loadthis=quest&QUEST=1\"><img src=\"images/quest_headerL.jpg\" width=\"255\" height=\"49\" border=\"0\"></a>"; }
			$cualCSS = "<LINK REL=\"STYLESHEET\" HREF=\"common_css.cgi?QUEST=1\" Type=\"text/css\">";
	...
	$menu2 = "<script language=\"Javascript\" src=\"js/newmenu_constructor.js\"></script>";
	...
	if ($FORM{'TPP'} eq "1") { 
		...
		if ($FORM{'loadthis'}) {
			## IF CALLING FOR NEW INTERFACE
			if ($FORM{'newIface'} eq "true") { 
				$loadthis = "stank_startpage_real.nsp";
				$FORM{'Series'} = $FORM{'loadthis'};
			}
			## IF SYSTEM, LOAD SYSTEM START PAGE
			elsif ($FORM{'loadthis'} eq "thresher" || $FORM{'loadthis'} eq "thresher2" || $FORM{'loadthis'} eq "tiger" || $FORM{'loadthis'} eq "gw" || $FORM{'loadthis'} eq "reef" || $FORM{'loadthis'} eq "mako" || $FORM{'loadthis'} eq "nmusd" || $FORM{'loadthis'} eq "nmusdCustCom" || $FORM{'loadthis'} eq "tpp" || $FORM{'loadthis'} eq "seagate" || $FORM{'loadthis'} eq "quest") { 
				$loadthis = "stank_startpage_real.nsp";
				$FORM{'Series'} = $FORM{'loadthis'};
			}
	...
	if($title eq "tpp" || $title eq "seagate" || $title eq "quest") {
		...
		$LEASING = "<div style=\"position:absolute;top:425px;left:15px;\"><a href=\"http://www.credentrust.com/pbo/nasba/vendorLand1.cfm?salespersonNumber=01000\" target=\"LEASING\"><img src=\"images/leasing_small.gif\" width=\"175\" height=\"67\" border=\"0\"  alt=\"Click to view more information about our system leasing option.\"></a></div>";
	}
	elsif ($FORM{'newIface'} eq "true" && $FORM{'loadsys'}) { 
		$whichdrop = `cat content/products/$FORM{'loadsys'}.txt`;
	}
	elsif ($FORM{'newIface'} eq "true" && $FORM{'loadthis'}) { 
		require ($loadthis);
		$THETABLE = $pageDrop;
		$whichdrop = "!!!MYCONTENT!!!";
		$whichdrop =~ s/!!!MYCONTENT!!!/$THETABLE/;
...
<script langauge="JavaScript" src="js/overlib2.js"></script>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" background="images/newones/2k3/top_tablebg.jpg" height="100">
  <tr> 
    <td width="30%"><img src="images/newones/2k3/$cualIllus" width="250" height="100" border="0"></td>
    <td align="right" valign="middle" width="70%"><font size="1" color="#ffffff" face="verdana,arial,helvetica">$date&nbsp;&nbsp;<font size="3">&nbsp;</font></font>
	<center><img src="images/newones/2k3/title_$imagetitle.jpg" width="350" height="80" align="bottom"></center></td>
  </tr>
</table>
<!-- End header table -->
