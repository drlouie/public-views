...
## If SAVING ##
###############
if (($FORM{'DROPIN'}) && ($FORM{'InternalCustNum'}) && ($FORM{'CompanyName'})) {
...
&topper;
	...
	my $SeeSeeT = "$FORM{'SeeSeeT'}";	
	my $SeeSeeN = "$FORM{'SeeSeeN'}";	
	my $SeeSeeE = "$FORM{'SeeSeeE'}";	
...
EOF
	...
	my $sth = $dbh->prepare("UPDATE LOW_PRIORITY Customers 
							 ...
							 ModifiedOn=Null 
							 WHERE InternalCustomerNumber='$InternalCustNum'");
...
## If DELETING ##
#################
elsif (($FORM{'DELETEIT'}) && ($FORM{'InternalCustNum'}) && ($FORM{'CompanyName'})) {
...
EOF
	...
	my $dbh = DBI->connect("DBI:mysql:NMS-SysConfigurator","root","62Ak.fi1HdBxI") or die "Unable to connect to database: <b>NMS-SysConfigurator</b>\n"; 
	$dbh->{RaiseError} = 1; 
	my $sth = $dbh->prepare("DELETE FROM Customers WHERE InternalCustomerNumber='$InternalCustNum'");
	$sth->execute or die "Unable to execute query\n"; 
	$sth->finish;
	my $sth = $dbh->prepare("DELETE FROM Users WHERE CustomerID='$InternalCustNum'");
	$sth->execute or die "Unable to execute query\n"; 
	$sth->finish;
	$dbh->disconnect;
...
## If VIEWING ##
################
elsif ($FORM{'InternalCustNum'} && $FORM{'CompanyName'}) {
	...
	$dbh->{RaiseError} = 1; 
	my $count=0;
	my $sth = $dbh->prepare("SELECT * FROM Customers WHERE InternalCustomerNumber='$InternalCustNum'");
	$sth->execute or die "Unable to execute query\n"; 
	my @row;
	while(@row = $sth->fetchrow_array) { 
		my $SavedInternalCustNum = $row[1];
		...
		## GET SAVED CC INFO
		my $SavedSeeSeeT = $row[16];
		my $SavedSeeSeeN = $row[17];
		my $SavedSeeSeeE = $row[18];
		my $SavedAddedBy = $row[19];
		my $SavedAddedOn = $row[20];
		my $SavedModifiedBy = $row[21];
		my $SavedModifiedOn = $row[22];
		my $SavedDeletedBy = $row[23];
		my $SavedDeletedOn = $row[24];

		my $SavedPayPi = $row[25];
		my $SavedBillingInformation = $row[26];
		my $SavedLastOrderDate = $row[27];
		my $SavedCustomerServiceInfo = $row[28];
		my $SavedTechnicalSupportInfo = $row[29];
		my $SavedCustomerSince = $row[30];
...
if (document.change.InternalCustNum.value == "" || document.change.InternalCustNum.value == " ") {
    missingdrop += "\\n     - Internal Customer Number";
	if (thefirst == "") { thefirst = "InternalCustNum"; }
}
...
for(var i = 0; i < ShippingID.length; i++) {
	if ((ShippingID.options[i] != null) && (ShippingID.options[i].selected)) {
		location.href = "change_shipdest.cgi?DELETEIT=1&ShippingID="+ShippingID.options[i].value+"&InternalCustNum=$SavedInternalCustNum&CompanyName=$SavedCompanyName";
	}
  }
}
...
for(var i = 0; i < ShippingID.length; i++) {
	if ((ShippingID.options[i] != null) && (ShippingID.options[i].selected)) {
		parent.location.href = "change_shipdest.cgi?ShippingID="+ShippingID.options[i].value+"&InternalCustNum=$SavedInternalCustNum&CompanyName=$SavedCompanyName";
	}
  }
}
                            ...
                            <tr> 
                              ...
                              <td width="348" valign="top"> 
                                <table width="300" border="0" cellspacing="0" cellpadding="0">
                                  <tr> 
                                    <td width="15" height="15" bgcolor="#8F8FAB">&nbsp;</td>
                                    <td height="20" bgcolor="#8F8FAB"><font face="verdana,arial,helvetica" size="1" color="#333366"><b><font color="#FFFFFF">Customer Number</font></b></font></td>
                                    <td width="15" height="15" bgcolor="#8F8FAB">&nbsp;</td>
                                  </tr>
                                  <tr> 
                                    <td colspan="3" height="1"><img src="images/verticalbar.gif" width="15" height="1"></td>
                                  ...
                                  <tr> 
                                    <td width="15" bgcolor="#F2F2F7">&nbsp;</td>
                                    <td height="35" align="center"> 
                                      <input type="text" size="15" class="inputtext15" name="InternalCustNum" value="$SavedInternalCustNum">
                                    </td>
                                    <td width="15" bgcolor="#F2F2F7">&nbsp;</td>
                                  </tr>
                                  <tr> 
                              ...
                              <td width="348" valign="top"> 
                                <table width="300" border="0" cellspacing="0" cellpadding="0">
                                  ...
                                  <tr>
                                    <td width="15" bgcolor="#F2F2F7">&nbsp;</td>
                                    <td height="30" align="center" valign="middle">
                                      <input type="button" value="Create New Location" class="inputbut" onClick="javascript:parent.location.href='add_shipdest.cgi?enformthee=1&InternalCustNum=$InternalCustNum';" name="button23">
                                    </td>
                                    <td width="15" bgcolor="#F2F2F7">&nbsp;</td>
                                  </tr>
                                  <tr> 
                                ...
                                <table width="300" border="0" cellspacing="0" cellpadding="0">
                                  ...
                                  <tr> 
                                    ...
                                    <td height="135" bgcolor="#F2F2F7" align="left" valign="middle"><font face="verdana,arial,helvetica" size="1" color="#333366">This 
                                      ...
                                      as many company representatives 
                                      ...
                                      company representatives in the system. 
                                      This feature only reads in users with COManage 
                                      or COSales rights.</font></td>
                                    <td width="15" bgcolor="#F2F2F7" height="15">&nbsp;</td>
                                  </tr>
                                  <tr> 
                                    <td width="15" bgcolor="#F2F2F7">&nbsp;</td>
                                  ...
                                  <tr> 
                                    ...
                                    <td height="45" align="center">
                                      <table width="280" border="0" cellspacing="0" cellpadding="0" bgcolor="#FFFFFF" align="center">
                                        ...
                                        <tr> 
                                          <td valign="top" align="center" width="130"> 
                                            <input type="Button" value="Add New User" onClick="javascript:location.href='add_user.cgi?enformthee=1&NewCompanyUser=1&InternalCustNum=$InternalCustNum&CompanyName=$CompanyName';" class="inputbut" name="Button2">
                                          </td>
                                ...
                                <table width="300" border="0" cellspacing="0" cellpadding="0">
                                  ...
                                  <tr> 
                                    ...
                                    <td height="125" bgcolor="#F2F2F7" align="left" valign="middle"><font face="verdana,arial,helvetica" size="1" color="#333366">The 
                                      ...
                                      a company Supervisor or Manager. 
                                ...
                                <table width="300" border="0" cellspacing="0" cellpadding="0">
                                  <tr> 
                                    ...
                                    <td height="20" bgcolor="#8F8FAB"><font face="verdana,arial,helvetica" size="1" color="#333366"><b><font color="#FFFFFF">Approved 
                                      ...
                                      (Web Store Administator Only)</font></font></td>
                                    <td width="15" height="15" bgcolor="#8F8FAB">&nbsp;</td>
                                  </tr>
                                  <tr> 
                                    <td colspan="3" height="1"><img src="images/verticalbar.gif" width="15" height="1"></td>
...
@Options = ("Cash","Check","CC","Net30","Net10","Net5");
                                      ...
                                      <table width="150" border="0" cellspacing="0" cellpadding="0">
                                        ...
                                        <tr> 
                                          <td width="125" height="30" valign="top" align="center"> 
                                            <input type="text" name="SeeSeeN" size="15" class="inputtext15" value="$SavedSeeSeeN">
                                          </td>
                                        </tr>
                                        <tr> 
                                        ...
                                        <tr> 
                                          <td width="125" height="15" valign="top" align="center"> 
                                            <input type="text" name="SeeSeeT" size="15" class="inputtext15" value="$SavedSeeSeeT">
                                          </td>
                                        </tr>
                                        <tr> 
                                        ...
                                        <tr> 
                                          <td width="125" height="30" valign="top" align="center"> 
                                            <input type="text" name="SeeSeeE" size="7" class="inputtext7" value="$SavedSeeSeeE">
                                          </td>
                                        </tr>
                                      </table>
...
## Get list REPS WHO ARE THIS CUSTOMERS REPS
...
while(@row = $sth->fetchrow_array) { 
	...
	if (($UserAccess eq "COManage") || ($UserAccess eq "WEBAdmin") || ($UserAccess eq "COSales")) {
...
## Get list of ALL reps
my $sth = $dbh->prepare("SELECT * FROM Users WHERE UserAccess = 'COManage' OR UserAccess = 'WEBAdmin' OR UserAccess = 'COSales' ORDER BY LastName ASC");
$sth->execute or die "Unable to execute query\n"; 
my @row;
while(@row = $sth->fetchrow_array) { 
	my $UserID = $row[0];
...
## Get list of ALL reps
my $sth = $dbh->prepare("SELECT * FROM Users WHERE CustomerID='$InternalCustNum' ORDER BY LastName ASC");
$sth->execute or die "Unable to execute query\n"; 
my @row;
while(@row = $sth->fetchrow_array) { 
	my $UserID = $row[0];
...
my $sth = $dbh->prepare("SELECT * FROM Messages WHERE CustomerID='$InternalCustNum' ORDER BY AddedOn ASC");
$sth->execute or die "Unable to execute query\n"; 
my @row;
while(@row = $sth->fetchrow_array) { 
	my $MessageID = $row[0];
...
my $sth = $dbh->prepare("SELECT * FROM ShipDests WHERE CustomerID='$InternalCustNum' ORDER BY AddedOn ASC");
$sth->execute or die "Unable to execute query\n"; 
my @row;
while(@row = $sth->fetchrow_array) { 
	my $ShipID = $row[0];
...
// Grab selected fields from PayOption and make PayTypes form field for processing
var formindex = document.change.InternalCustNum.selectedIndex;
var thisone = document.change.InternalCustNum.options[formindex].text;
...
return true;
                                        ...
                                        <select name="InternalCustNum" class="inputtext">
EOF
}
                                        ...
                                        <table border="1" cellspacing="0" cellpadding="0" width="200" bordercolor="#8F8FAB">
                                          ...
                                          <tr> 
                                            <td height="125" width="200"> 
                                              <input type="radio" name="SearchType" value="InternalCustNum" SELECTED checked>
                                              <font face="verdana,arial,helvetica" size="1" color="#333366">Customer Number</font><br>
                                        ...
                                        <table border="1" cellspacing="0" cellpadding="0" width="200" bordercolor="#8F8FAB">
                                          ...
                                          <tr> 
                                            <td width="200" height="125"> 
                                              <input type="radio" name="Browse" value="InternalCustNum" SELECTED checked>
                                              <font face="verdana,arial,helvetica" size="1" color="#333366">Customer Number</font><br>
