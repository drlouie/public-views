...
$muasearch = "http://www.rhomberg.com/systemConfigurator/admin/s_products.cgi";
...
while(@row = $sth->fetchrow_array) { 
	...
	$SavedCost = $row[21];
	$files[$count] = "$SavedInternalPartNum-----$SavedMFGPartNum-----$SavedItemName------$SavedDescription------$SavedKeywords------$SavedCost" . "\t" . "$SavedInternalPartNum" . "\t" . "$SavedItemName" . "\t" . "$SavedMFGCode" . "\t" . "$SavedMFGPartNum" . "\t" . "$SavedKITCode" . "\t" . "$SavedSYSCode" . "\t" . "$SavedCost";
...
foreach $item (@sresults2)	
	...
	if (($ippc > $fields{'st'}) and ($ippc <= $fields{'nd'}))
		...
		if ($syscode ne "") { $myType = "change_product_system.cgi"; $myDel="change_product_system.cgi?DELETEIT=1&InternalPartNum=$iurl";}
		...
		$wls =~ s/!!Title_With_Link!!/<a href="$myType\?InternalPartNum=$iurl"><font class="slink">$ptitle<\/font><\/a>/gi;
		$wls =~ s/!!CLM!!/$iurl/gi;
