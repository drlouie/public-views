...
EOF
	...
	my $sth = $dbh->prepare("UPDATE LOW_PRIORITY Systems 
							 ...
							 ModifiedBy='$ActionUser', 
							 ModifiedOn=Null 
							 WHERE Parent='$InternalPartNum'");
	$sth->execute or die "Unable to execute query\n"; 
	$sth->finish; 
	$dbh->disconnect; 	
	print "<center><br><b>Inventory Updated</b><br><br>The the changes you have made to <b>$ItemName ($InternalPartNum)</b> have been successfully saved to the system.<br><br><form><input type=\"button\" value=\"Back to System: $InternalPartNum\" onClick=\"javascript:location.href='change_product_system.cgi?InternalPartNum=$InternalPartNum'\" class=\"inputbut\"><br><br><input type=\"button\" value=\"Inventory Main\" onClick=\"javascript:parent.location.href='change_product.cgi'\" class=\"inputbut\"></form><br></center><br><br></font>";
...
EOF
	...
	$sth->execute or die "Unable to execute query\n"; 
	$sth->finish;
	my $sth = $dbh->prepare("DELETE FROM Systems WHERE Parent='$InternalPartNum'");
	$sth->execute or die "Unable to execute query\n"; 
	$sth->finish;
...
## If VIEWING
	...
	$dbh->{RaiseError} = 1; 
	$count=0;
	my $sth = $dbh->prepare("SELECT * FROM Products WHERE InternalPartNum='$InternalPartNum' AND SYSCode != 'NULL'");
	$sth->execute or die "Unable to execute query\n";
	my @row;
              ...
              <tr bordercolor="#333366"> 
                <td width="50%" align="center" valign="top" height="250">
                  <table width="65%" border="0" cellspacing="0" cellpadding="5">
                    <tr>
                      <td width="100%"><font face="verdana,arial,helvetica" size="1" color="#333366">
<br><b>No Matching Records Found</b><ul><li>The Part Number you specified does not seem to be a <b>System</b>, as you specified.</li><li>Are you sure <b>$InternalPartNum</b> isn't actually a stand-alone item in the system and not a system?</li></ul><form><center><input type="button" value="Try Again" onClick="javascript:history.go('-1')" class="inputbut"></center><br>
<br><br></font>
                      </td>
                    </tr>
                  </table>
...
##----------->>> ELSE CONTINUE
	...
	my @row;
	while(@row = $sth->fetchrow_array) { 
		$SavedSystemID = $row[0]; $SavedParent = $row[1]; $SavedSystemSeries = $row[2]; $SavedReconfig = $row[3];
...
<html>
<head>
<title>Shark Tank Admin System - Inventory ( Systems )</title>
...
function editSystemItem(cual) {
	var InternalPartNum = document.change[cual].value;
	var esteURL = 'change_product_common.cgi?InternalPartNum='+InternalPartNum+'';
	if (InternalPartNum != "" && InternalPartNum != "null") {
...
EOF
	...
	my $dbh = DBI->connect("DBI:mysql:NMS-SysConfigurator","root","62Ak.fi1HdBxI") or die "Unable to connect to database: <b>NMS-SysConfigurator</b>\n"; 
	$dbh->{RaiseError} = 1; 
	my $sth = $dbh->prepare("SELECT * FROM SystemComponents ORDER BY ComponentName ASC");
	$sth->execute or die "Unable to execute query\n"; 
	my @row;
		while(@row = $sth->fetchrow_array) { 
		my $CID = $row[0];
		my $ComponentName = $row[1];
		my $ComponentType = $row[2];
		push(@allComponents," || (thisone == \"$CID\")");
	}
	$sth->finish;
	$dbh->disconnect;
	## print closing
...
print <<EOF
	...
	if ((thisone == "Desktop") || (thisone == "Server") || (thisone == "Notebook") || (thisone == "Kit") @allComponents) {
		var b = navigator.appName;
		if (b=="Netscape") {
			mainFrame = parent.frames.botOne;
...
EOF
	...
	my @row;
		...
		my $ComponentName = $row[1];
		my $ComponentType = $row[2];
		print "TheType.options[sizer] = new Option('$ComponentName'); TheType.options[sizer].value = '$CID'; sizer++;\n";
	}
	$sth->finish;
	$dbh->disconnect;
...
else { location.href = "$script?InternalPartNum=$InternalPartNum&AddComponent=1"; }
}
        ...
        <table width="95%" border="0" cellspacing="0" cellpadding="0">
          <tr> 
            <td width="95%"> <font face="verdana,arial,helvetica" size="2" color="#333366"><b>View/Update 
              Product ($SavedPricingClass - $InternalPartNum)</b></font></td>
...
if ($SavedMainCharacter eq "All") { $All = "selected"; $Multiple = ""; $Single = ""; }
	...
	foreach $MainPart (@MainParts) {
		my $sth = $dbh->prepare("SELECT * FROM Products WHERE InternalPartNum='$MainPart' ORDER BY ItemName ASC");
		$sth->execute or die "Unable to execute query\n"; 
		my @row;
		while(@row = $sth->fetchrow_array) { 
		print "$EsteNombre";
...
if (($Numero eq $Counter) && ($FORM{'AddComponent'} eq "1")) {
	...
	foreach $SubPart (@SubParts) {
		my $sth = $dbh->prepare("SELECT * FROM Products WHERE InternalPartNum='$SubPart' ORDER BY ItemName ASC");
		$sth->execute or die "Unable to execute query\n"; 
		my @row;
		while(@row = $sth->fetchrow_array) { 
			$MiEsteNombre = $row[11];
...
if (agree)
location.href = "change_product_system.cgi?AddComponent=1&InternalPartNum=$InternalPartNum";
else
nogo=1;
}
</script>
<input type="button" value="Add Another Component" class="inputbut" onClick="javascript:addComponent();">
                                                ...
                                                <td width="50%" height="35"><font face="verdana,arial,helvetica" size="1">
                                                  <input type="button" value="Preview As Configured" class="inputbut" name="button3" onClick="Javascript:window.open('../stank_sysbrowse.cgi?InternalPartNum=$InternalPartNum','Preview','width=470,height=590');">
                                                  </font></td>
                                              </tr>
                                            </table>
                                          </td>
