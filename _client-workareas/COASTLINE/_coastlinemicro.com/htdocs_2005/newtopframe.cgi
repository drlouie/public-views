...
if ($Cookies{'GID'} && ($Cookies{'UTipe'} eq "CustAdmin" || $Cookies{'UTipe'} eq "CustUser")) {
	...
	$Welcome = "<b>Welcome to our storefront $Cookies{'FiNo'} $Cookies{'LaNo'}...</b><br><a href=\"logoff.cgi\" target=\"_parent\">I am not $Cookies{'FiNo'}!</a>";
	$LoginBut = "<a href=\"logoff.cgi\" onClick=\"document.images.search.src = 'images/spacer.gif';\" target=\"botOne\" onMouseOver=\"javascript:imageOn('logoff','on');\" onMouseOut=\"javascript:imageOff('logoff','off');\"><img src=\"images/dbing/button_logoff_off.gif\" width=\"75\" height=\"19\" name=\"logoff\" border=\"0\"></a>";
}
else {
...
if($FORM{'mymark'} && $FORM{'ourcost'} && $FORM{'quantity'}) {
	$MyTypo = 100 - $FORM{'mymark'};
	if ($MyTypo < 10) { $MyTypo = "." . "0" . "$MyTypo"; }
	else { $MyTypo = ".$MyTypo"; }
	$Markup = ($FORM{'ourcost'} / $MyTypo);
	$MarkedUp = sprintf("%.2f", $Markup);
	
	$theMarkUp = $MarkedUp - $FORM{'ourcost'};
	$theMarkUp = sprintf("%.2f", $theMarkUp);


	$totalprice = $MarkedUp * $FORM{'quantity'};
	$totalprice = sprintf("%.2f", $totalprice);

	print "
			function newPrice() {
				parent.frames.botOne.leConfig.MarkedUp.value = \"$MarkedUp\";
				parent.frames.botOne.leConfig.HowMuchMarked.value = \"$theMarkUp\";
				parent.frames.botOne.leConfig.TotalPrice.value = \"$totalprice\";
			}
	...
	$myPriceLoader = "<script>newPrice();</script>";
}
