...
$muasearch = "ss_prod.html";

&get_env;
print "Content-type: text/html\n\n";

$fsize = (-s "ss_prod_ret.nsf");
	open (RVF, "ss_prod_ret.nsf");
		read(RVF,$mainlayout,$fsize);
	close (RVF);

$fsize = (-s "ss_prod_list.nsf");
	open (RVF, "ss_prod_list.nsf");
		read(RVF,$listings,$fsize);
	close (RVF);


if ($fields{'SSResults'}) { $listperpage = "$fields{'SSResults'}"; }
else { $listperpage = "10"; }

if ($fields{'SSOrder'} eq "OName") { $leSort = "ItemName"; }
else { $leSort = "ProductType"; }
	
if ($fields{'SSKeys'} eq "" && $fields{'SSMFG'} eq "BIGNULL" && $fields{'SSMFGType'} eq "" && $fields{'SSCLMNum'} eq "" && $fields{'SSMFGNum'} eq "") { 
	$fields{'SSINKeys'} = "$fields{'SSKeys'}";
	$fields{'SSKeys'} = "No&nbsp;search&nbsp;terms&nbsp;given";
}
else {
	$fields{'SSINKeys'} = "$fields{'SSKeys'}";
	$fields{'SSKeys'} = "$fields{'SSKeys'}"; 
}

$RESearch = "SSResults=$fields{'SSResults'}&SSType=$fields{'SSType'}&SSMFGNum=$fields{'SSMFGNum'}&SSCLMNum=$fields{'SSCLMNum'}&SSMFG=$fields{'SSMFG'}";

if ($fields{'SSType'} ne "BIGNULL" && $fields{'SSType'} ne "All" && $fields{'SSType'} ne " " && $fields{'SSType'} ne "") { push(@MuaQuery,"ProductType,,,,,=,,,,,$fields{'SSType'}"); }
if ($fields{'SSMFGNum'} ne "" && $fields{'SSMFGNum'} ne " ") { push(@MuaQuery,"MFGPartNum,,,,,LIKE,,,,,%$fields{'SSMFGNum'}%"); }
if ($fields{'SSCLMNum'} ne "" || $fields{'SSCLMNum'} ne " ") { push(@MuaQuery,"CMPartNum,,,,,LIKE,,,,,%$fields{'SSCLMNum'}%"); }
if ($fields{'SSMFG'} ne "0000000001" && $fields{'SSMFG'} ne "BIGNULL") { push(@MuaQuery,"MFGCode,,,,,=,,,,,$fields{'SSMFG'}"); }
elsif ($fields{'SSMFG'} ne "BIGNULL") { push(@MuaQuery,"MFGCode,,,,,=,,,,,0000000001"); }

## generate query depending on how many items...
$QCount=0;
foreach $QPart (@MuaQuery) {
	$QCount++;
	@QParts = split (/,,,,,/,$QPart);
	if ($QCount eq "1")	{ $leQuery = "WHERE $QParts[0] $QParts[1] '$QParts[2]'"; }
	else { $leQuery = "$leQuery" . " AND $QParts[0] $QParts[1] '$QParts[2]'"; }
}
...
my $sth = $dbh->prepare("SELECT * FROM Products $leQuery ORDER BY $leSort DESC");
...
if ($fields{'SSMFG'} eq "0000000001") {
	my $sth = $dbh->prepare("SELECT * FROM Products WHERE SYSCode != 'NULL' ORDER BY $leSort DESC");
	$sth->execute or die "Unable to execute query\n"; 
	my @row;
	while(@row = $sth->fetchrow_array) { 
		$SavedCMPartNum = $row[1];
...
### PREPARE KEYWORDS
if ($fields{'SSKeys'} =~ / /) { @keywords = split (/ /,$fields{'SSKeys'}); }
else { $keywords[0] = $fields{'SSKeys'}; }
	...
	foreach $kw (@keywords) {
		if ($item =~ /$kw/i) {
			...
			foreach $oln (@filelines) {	
				...
				if (($oln =~ /$fields{'SSKeys'}/i) and ($fields{'SSKeys'} =~ " "))
...
if ($fields{'SSImages'} eq "YES") { $PC1 = "15"; $PC2 = "65"; $ImageTitle = "Image"; $GETIMAGES = "YES"; }
else { $PC1 = "1"; $PC2 = "84"; $ImageTitle = "&nbsp;"; }
...
$mainlayout =~ s/!!Keywords!!/$fields{'SSINKeys'}/gi;
$legal = `cat legal.nsf`;
...
$MiCount++;
	...
	if (($ippc > $fields{'st'}) and ($ippc <= $fields{'nd'}))
		...
		$wls =~ s/!!CLM!!/$iurl/gi;
		if ($fields{'SSDescrip'} eq "YES") { $wls =~ s/!!ProdDesc!!/<br>$leDesc/gi; }
		else { $wls =~ s/!!ProdDesc!!//gi; }
  ...
  for ($ms = 0; $ms < $pages; $ms++) 
	...
	if ($fields{'nd'} == ($pg * $listperpage))
		...
		$nd = ($pg * $listperpage);
		$pgstring = $pgstring . "<a href=\"$muasearch?SSKeys=$fields{'SSINKeys'}&st=$st&nd=$nd&SSImages=$fields{'SSImages'}&SSDescrip=$fields{'SSDescrip'}&$RESearch\"><font class=\"slink\">$pg</font></a> ";
		}
	}
...
if ($spls == 0){$spls++;}
 ...
 if ($fields{'nd'} <= ($allresults - $spls))
	...
	$nextt = "<a href=\"$muasearch?SSKeys=$fields{'SSINKeys'}&st=$st&nd=$nd&SSImages=$fields{'SSImages'}&SSDescrip=$fields{'SSDescrip'}&$RESearch\"><font class=\"slink\">Next Page >></font></a> ";
	}
 ...
 if ($fields{'st'} > 0)
		...
		$st = $fields{'st'} - $listperpage;
		$nd = $fields{'nd'} - $listperpage;
		$prev = "<a href=\"$muasearch?SSKeys=$fields{'SSINKeys'}&st=$st&nd=$nd&SSImages=$fields{'SSImages'}&SSDescrip=$fields{'SSDescrip'}&$RESearch\"><font class=\"slink\"><< Prev Page</font></a> ";
		}
...
## -------------->>> KILL DB CONNECTION FOR THIS DOC
$dbh->disconnect;
