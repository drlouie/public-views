...
if (($FORM{'Username'}) && ($FORM{'Password'}) && ($FORM{'Email'}) && ($FORM{'FirstName'}) && ($FORM{'LastName'}) && ($FORM{'CMCustNum'}) && ($FORM{'UserAccess'})) { 
...
## ------------->>> ELSE
	...
	my $sth = $dbh->prepare("INSERT INTO Users (UserID, CustomerID, EmailType, 
							...
							ExtraAccess)
						 	...
						 	VALUES (Null, '$CustomerID', Null, '$Username', 
							'$Password', '$Email', '$FirstName', '$LastName', 
							'$Position', '$Phone', '$Extension', '$UserAccess', 
							'$ActionUser', Null, '', '', '', '', '$ExtraAccess')");
	$sth->execute or die "Unable to execute query\n"; 
	$sth->finish; 
	$dbh->disconnect; 
...
$GREETING = "$FirstName,\n\n";
$BODY = "A user account has just been created for you in the Coastline Micro Shark Tank, our E-Com store. Following is the login information for that account:\n\nUsername - $Username\n\nPassword - $Password\nClick here to access the Shark Tank: http://www.coastlinemicro.com/\n\n\n";
$ATTACHMENTS = "";
$FOOTER = "Shark Tank Administration System\nhttp://www.coastlinemicro.com/\n\nShark Tank ECom Store\nhttp://www.coastlinemicro.com/\n\n\n";
$MYHTMLTITLE = "<title>Coastline Micro, Inc. - Shark Tank Admin System - New User Account Information</title>";
$MYHTMLSTAMP = "<table width=\"400\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\" align=\"center\"><tr><td width=\"100%\" valign=\"top\"><font face=\"verdana,arial,helvetica\" size=\"1\" color=\"#333366\">$FirstName,<br><br>A user account has just been created for you in the Coastline Micro Shark Tank&#153;, our E-Com store. Following is the login information for that account:<br><br><b>Username</b> - $Username<br><b>Password</b> - $Password<br><br><ul><li>Shark Tank&#153; ECom Store<br><a href=\"http://www.coastlinemicro.com/\">http://www.coastlinemicro.com/</a></li><li>Did not request this info?<br><a href=\"mailto:security\@coastlinemicro.com?Subject=Bad User Addition (Shark Tank)\">Report It</a></li></ul></font></td></tr></table>";
$MYHTMLATTACH = "";
...
## ----------------------->>> END EMAILINGS
	...
	my $SenderIn = "stadmin\@coastlinemicro.com(Coastline Micro, Inc. - Shark Tank)";
	my $SubjectIn = "Administration System - New User Account Information";
	my $BodyIn = "$FirstName,\n\nA user account has just been created for you in the Shark Tank administrative system. Following is the login information for that account:Username - $Username\n\nPassword - $Password\nClick here to access the administration system: http://www.coastlinemicro.com/\n\n\n";
	my $Attach1In = "";
	my $Attach2In = "";
	my $Attach3In = "";
	## Start DB connection
...
<html>
<head>
<title>Coastline Micro, Inc. - Shark Tank Admin System - Add User</title>
<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1">
<LINK REL="STYLESHEET" HREF="http://www.coastlinemicro.com/admin/admincss.cgi" Type="text/css">
...
if (($Cookies{'UserType'} eq "CMManage") || ($Cookies{'ExtraAccess'} =~ "ExtraAccess")) {
...
if (($Cookies{'UserType'} eq "CMManage") || ($Cookies{'UserType'} eq "CMAdmin") || ($Cookies{'ExtraAccess'} =~ "ExtraAccess")) { $next=1; }
...
if (($Cookies{'UserType'} eq "CMManage") || ($Cookies{'UserType'} eq "CMAdmin") || ($Cookies{'ExtraAccess'} =~ "PayOptions")) { print "<body TEXT=\"#333366\" LINK=\"#8F8FAB\" VLINK=\"#8F8FAB\" ALINK=\"#8F8FAB\" BGCOLOR=\"#ffffff\" leftmargin=\"0\" topmargin=\"0\" marginwidth=\"0\" marginheight=\"0\">"; }
## -------------------------->>> WITHOUT
...
## ------------->>> IF AddAdmin SPECIFIED then use that CompanyName and CMCustNum - FOR USERS ADDING NEW CUSTOMER ACCOUNT WITH ADMIN USER
if ($FORM{'AddAdmin'} && $FORM{'CMCustNum'} && $FORM{'CompanyName'}) {
...
## ------------->>> IF NewCompanyUser SPECIFIED then use that CompanyName and CMCustNum - FOR ADDING NEW USERS TO EXISTING COMPANY PROFILE
elsif ($FORM{'NewCompanyUser'} && $FORM{'CMCustNum'} && $FORM{'CompanyName'}) {
...
if ($FORM{'AddAdmin'} && $FORM{'CMCustNum'} && $FORM{'CompanyName'}) {
	...
	my $CompanyName = "$FORM{'CompanyName'}";
	print "<input type=\"hidden\" value=\"$CMCustNum\" name=\"CMCustNum\"><input type=\"hidden\" value=\"$CompanyName\" name=\"CompanyName\"><input type=\"hidden\" value=\"1\" name=\"AddAdmin\"><font face=\"verdana,arial,helvetica\" size=\"1\" color=\"#EB0000\">$CompanyName</font><!--PARENT COMPANY-->";
}
## ------------->>> IF NewCompanyUser SPECIFIED then use that CompanyName and CMCustNum - FOR ADDING NEW USERS TO EXISTING COMPANY PROFILE
elsif ($FORM{'NewCompanyUser'} && $FORM{'CMCustNum'} && $FORM{'CompanyName'}) {
	my $CMCustNum = "$FORM{'CMCustNum'}";
	my $CompanyName = "$FORM{'CompanyName'}";
	print "<input type=\"hidden\" value=\"$CMCustNum\" name=\"CMCustNum\"><input type=\"hidden\" value=\"$CompanyName\" name=\"CompanyName\"><input type=\"hidden\" value=\"1\" name=\"NewCompanyUser\"><font face=\"verdana,arial,helvetica\" size=\"1\" color=\"#EB0000\">$CompanyName</font><!--PARENT COMPANY-->";
}
## ------------->>> ELSE IF CUSTOMER NOT SPECIFIED then grab all Customers from system filtering CM test Customer accounts out
else {
	my $CMCustNum = "$FORM{'CMCustNum'}";
	my $CompanyName = "$FORM{'CompanyName'}";
	print "<select name=\"CMCustNum\" width=\"20\" class=\"inputtext\">";
	## Get list of CM reps
	use DBI;
	my $dbh = DBI->connect("DBI:mysql:coastline","drlouie","chinga2") or die "Unable to connect to database: <b>coastline</b>\n"; 
	$dbh->{RaiseError} = 1; 
...
if ($FORM{'CMCustNum'} && $FORM{'CompanyName'} ) { print "<option value=\"CustUser\" SELECTED>Customer User</option><option value=\"CustAdmin\">Customer Admin</option>"; }
## ------------->>> ELSE IF CUSTOMER NOT SPECIFIED
else { print "<option value=\"CustUser\" SELECTED>Customer User</option><option value=\"CustAdmin\">Customer Admin</option><option value=\"CMUser\">CM User</option><option value=\"CMSales\">CM Sales</option><option value=\"CMAdmin\">CM Admin</option><option value=\"CMManage\">CM Manage</option>"; }
...
<html>
<head>
<title>Coastline Micro, Inc. - Shark Tank Admin System - User Accounts</title>
</head>
<frameset rows="64,*" rows="*" border="0" framespacing="0"> 
<frame name="topOne" scrolling="NO" noresize src="http://www.coastlinemicro.com/admin/topper.cgi" marginwidth="0" marginheight="0" frameborder="NO">
