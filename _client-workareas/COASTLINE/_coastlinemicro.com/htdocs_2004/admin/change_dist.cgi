...
## If SAVING
if (($FORM{'DROPIN'}) && ($FORM{'Name'}) && ($FORM{'DISTID'})) {
...
&topper;
	...
	$Name = "$FORM{'Name'}";
	$URL = "$FORM{'Url'}";
	$MailAdd = "$FORM{'MailAdd'}";
	$MailAdd2 = "$FORM{'MailAdd2'}";
...
EOF
	...
	print "<center><br><b>Update Distributor Profile</b><br><br>The profile for <b>$Name\</b> has been updated successfully.<br><br><form><input type=\"button\" value=\"Back to $Name\'s Profile\" onClick=\"javascript:location.href='change_dist.cgi?DISTID=$DISTID\&Name=$Name\'\" class=\"inputbut\"><br><br><input type=\"button\" value=\"Back to Main Screen\" onClick=\"javascript:parent.location.href='index.cgi'\" class=\"inputbut\"><br></form></center>";
	print "<br><br></font>";
	my $dbh = DBI->connect("DBI:mysql:coastline","drlouie","chinga2") or die "Unable to connect to database: <b>coastline</b>\n"; 
	$dbh->{RaiseError} = 1; 
	my $sth = $dbh->prepare("UPDATE LOW_PRIORITY DISTs 
							 SET URL='$URL', 
							 MailAdd='$MailAdd', 
							 MailAdd2='$MailAdd2', 
							 MailCity='$MailCity', 
...
## If DELETING
elsif (($FORM{'DELETEIT'}) && ($FORM{'Name'}) && ($FORM{'DISTID'})) {

## TOP HTML TEMPLATE
&topper;

	## Crumble Form Input to Variables
	my $DISTID = "$FORM{'DISTID'}";
	my $Name = "$FORM{'Name'}";
...
EOF
	...
	print "<center><br><b>Delete Distributor Profile</b><br><br>The profile for <b>$Name</b> has been deleted from the system effective immediately.<br><br><form><input type=\"button\" value=\"Back to Home\" onClick=\"javascript:parent.location.href='index.cgi'\" class=\"inputbut\"><br></center>";
	print "<br><br></font>";
	my $dbh = DBI->connect("DBI:mysql:coastline","drlouie","chinga2") or die "Unable to connect to database: <b>coastline</b>\n"; 
	$dbh->{RaiseError} = 1; 
	my $sth = $dbh->prepare("DELETE FROM DISTs WHERE DistributorID=$DISTID AND Name='$Name'");
	$sth->execute or die "Unable to execute query\n"; 
	$sth->finish;
	$dbh->disconnect;
...
## If VIEWING
	...
	$dbh->{RaiseError} = 1; 
	my $count=0;
	my $sth = $dbh->prepare("SELECT * FROM DISTs WHERE Name='$Name'");
	$sth->execute or die "Unable to execute query\n"; 
	my @row;
	while(@row = $sth->fetchrow_array) { 
		my $SavedDISTID = $row[0];
		my $SavedName = $row[1];
		my $SavedUrl = $row[2];
		...
		my $SavedEmblem = $row[11];
		my $SavedSmallLogo = $row[12];
		my $SavedMediumLogo = $row[13];
		my $SavedLargeLogo = $row[14];
...
##----------->>> START DYNAMO WINDOW STUFF
	...
	$divcont = "#divCont{position:absolute; overflow:hidden; left:25%; top:955; clip:rect(0,300,450,0); height:450; width:300;}";
	$divload = "#divLoad{position:absolute; left:25%; top:955; clip:rect(0,300,450,0); height:450; width:300; visibility:hidden;}";	
	$divArrows = "#divArrows{position:absolute; left:0; top:1155; z-index:25; visibility:hidden}";
	$divBground = "#bground{position:absolute; left:0; top:1155; clip:rect(0,472,452,0); height:452; width:472;$zindex}";
...
if (b=="Netscape") {
	mainFrame = parent.frames.botOne;
	mainFrame.frame.loadpage('ichoose.cgi?FeedMe='+me+'&Type=dist&SubType='+me2+'');
}

else {
	parent.botOne.frame.loadpage('ichoose.cgi?FeedMe='+me+'&Type=dist&SubType='+me2+'');
	}
}
        ...
        <tr> 
          <td colspan="2"> 
              ...
              <tr bordercolor="#333366"> 
                  ...
                  <table width="100%" border="0" cellspacing="0" cellpadding="5">
                    <tr> 
                      <td width="100%" align="center" height="25" bgcolor="#F2F2F7" class="tableBG" colspan="2"><font face="verdana,arial,helvetica" size="2" color="#333366"><b>Distributor's 
                            ...
                            <td width="348" valign="top"> 
                              <table width="300" border="0" cellspacing="0" cellpadding="0">
                                <tr> 
                                  ...
                                  <td height="20" bgcolor="#8F8FAB"><font face="verdana,arial,helvetica" size="1" color="#333366"><b><font color="#FFFFFF">Distributor's 
                                ...
                                <tr> 
                                  ...
                                  <td height="35" align="center"> 
                                    <input type="text" name="Name" size="15" class="inputtext15" value="$SavedName">
                                    <input type="hidden" name="DISTID" value="$SavedDISTID">
                                  </td>
                                  <td width="15" bgcolor="#F2F2F7">&nbsp;</td>
                                </tr>
                                <tr> 
                              ...
                              <table width="300" border="0" cellspacing="0" cellpadding="0">
                                ...
                                <tr> 
                                  ...
                                  <td height="35" align="center"> <nobr>
                                    ...
                                    <input type="button" value="Preview" onClick="Javascript:iPreview('SmallLogo')" class="inputbut">
                                    </nobr> </td>
                                  <td width="15" bgcolor="#F2F2F7">&nbsp;</td>
                                </tr>
                                <tr> 
                            ...
                            <td width="348" valign="top"> 
                              <table width="300" border="0" cellspacing="0" cellpadding="0">
                                ...
                                <tr> 
                                  <td width="15" bgcolor="#F2F2F7">&nbsp;</td>
                                  <td height="35" align="center"> <nobr>
                                    <input type="text" name="MediumLogo" size="7" class="inputtext7" value="$SavedMediumLogo" onFocus="Javascript:noChange('MedLogoBut');">
                                    <input type="button" value="Get Mine!" onClick="Javascript:theHungry('MediumLogo','medium')" class="inputbut" name="MedLogoBut">
                                    <input type="button" value="Preview" onClick="Javascript:iPreview('MediumLogo')" class="inputbut">
                                    </nobr> </td>
                                  <td width="15" bgcolor="#F2F2F7">&nbsp;</td>
                                </tr>
                                <tr> 
...
<html>
<head>
<title>Coastline Micro, Inc. - Shark Tank Admin System - Distributor Profiles</title>
<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1">
<LINK REL="STYLESHEET" HREF="admincss.cgi" Type="text/css">
$MenuConfig
</head>
...
<html>
<head>
<title>Distributor Profiles</title>
<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1">
<LINK REL="STYLESHEET" HREF="admincss.cgi" Type="text/css">
<SCRIPT LANGUAGE="JavaScript">
