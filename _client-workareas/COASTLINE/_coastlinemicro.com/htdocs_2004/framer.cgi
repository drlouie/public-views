...
if (($FORM{'subject'} && $FORM{'title'}) || $FORM{'PKeywords'} || $FORM{'keywords'} ) { 
	...
	if ($FORM{'loadthis'} || $FORM{'loadprod'} || $FORM{'loadsys'} || $FORM{'loadkit'} || $FORM{'cartit'}) {
		...
		$botOne ="<frame name=\"botOne\" noresize src=\"products.html?title=$title$loadthis$loadprod$loadkit$loadsys$cartit$MSP$NMUSD\">";
	}

	### IF PRESS RELEASE CALL PRESENT MAKE - MAKE A FRAME AND CALL THE PRESS RELEASE AS NECESSARY
	elsif ($FORM{'viewRel'}) {
		$viewrel = "&viewRel=$FORM{'viewRel'}";
		$botOne ="<frame name=\"botOne\" noresize scrolling=\"Yes\" src=\"$subject.html?press.html&viewRel=$viewrel\">";
	}

	### IF HOMEPAGE SEARCH CALL
	elsif ($FORM{'PKeywords'}) {
		$botOne ="<frame name=\"botOne\" noresize scrolling=\"Yes\" src=\"s_prod.html?PKeywords=$FORM{'PKeywords'}&ProductType=All\">";
	}

	### IF HOMEPAGE SEARCH CALL
	elsif ($FORM{'keywords'}) {
		$botOne ="<frame name=\"botOne\" noresize scrolling=\"Yes\" src=\"s.html?keywords=$FORM{'keywords'}\">";
	}
	...
	else { $botOne ="<frame name=\"botOne\" noresize scrolling=\"Yes\" src=\"$subject.html?title=$title\">"; }

	if ($FORM{'PageTitle'}) { $Pagetitle = "Coastline Micro, Inc. - $FORM{'PageTitle'}"; }
	else { $Pagetitle = "Coastline Micro, Inc. - Welcome to the Shark Tank"; }
...
<frameset rows="64,*" frameborder="NO" border="0" framespacing="0" cols="*">
  <frame name="topOne" noresize scrolling="No" src="http://www.coastlinemicro.com/topframe.html">
  $botOne
</frameset>
<noframes>
<body bgcolor="#FFFFFF">
